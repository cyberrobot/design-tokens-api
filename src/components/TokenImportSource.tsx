      ...
      {activeTab === 0 && <div className="flex flex-col gap-6 px-4 py-8 bg-neutral rounded-md mt-5 shadow-sm">
        ...
        <div>
          ...
          <div className="join mt-2 w-full">
            <div className='join-item bg-base-100 border-base-300 border-[1px] rounded-md flex items-center px-3 pr-[1px]'>
              <FaGithub size={24} /><span className="ml-2">https://github.com/</span>
            </div>
            <input value={gitHubPath} onChange={(e) => setGithubPath(e.target.value)} type="text" placeholder="path/to/the/file" className="input input-bordered join-item pl-0 focus:outline-none border-l-0 w-full" />
            <button className="btn join-item" onClick={githubImportHandler}>Add {githubMutation.isLoading && <span className="loading loading-dots loading-sm"></span>}</button>
          </div>
