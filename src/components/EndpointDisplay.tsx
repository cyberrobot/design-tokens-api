...
export default function EndpointDisplay({ query }: { query: unknown }) {
  ...
  return (
    <div className='p-4'>
      ...
      <div ref={spanRef} className='relative border-[1px] border-base-200 rounded-md p-2 pr-14 text-sm break-all'>
        <div className="absolute top-0 right-0 inline-block">
          <CopyToClipboard content={url} styles='rounded-tr-md rounded-tl-none rounded-br-none btn-sm' />
        </div>
        {url}
      </div>
    </div>
