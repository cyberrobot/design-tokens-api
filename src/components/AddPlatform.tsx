...
export default function AddPlatform() {
  ...
  const onSubmitHandler = (event: FormEvent<HTMLFormElement>) => {
    ...
    const hasValues = nameRef.current?.value !== '' && transformGroupRef.current?.value !== '' && formatsRef.current?.value !== '';
    if (hasValues && nameRef.current && transformGroupRef.current && formatsRef.current) {
      addPlatform({
  ...
  return (
    <form className="flex gap-4" onSubmit={onSubmitHandler}>
      <input ref={nameRef} type="text" name="transform.name" className="input input-bordered flex-grow-1 w-[100%]" placeholder="Transform name" onChange={e => {
        if (nameRef.current) {
          nameRef.current.value = e.target.value
        }
      }} />
      <Dropdown<string> value={transformGroups} placeholder='Transform group' onSelect={selectedItems => {
        if (transformGroupRef.current) {
          transformGroupRef.current.value = selectedItems[0] || '';
        }
      }} defaultValue={transformGroupInState} />
      <input ref={transformGroupRef} type="hidden" name="transform.transformGroup" />
      <Dropdown<string> value={formats} multiSelect placeholder='Formats' onSelect={selectedItems => {
